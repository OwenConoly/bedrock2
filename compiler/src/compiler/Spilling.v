...
Section Spilling.
  ...
  Notation with_IO := Semantics.with_IO.
  Notation with_branch := Semantics.with_branch.
  Notation with_read := Semantics.with_read.
  Notation with_write := Semantics.with_write.
  Notation with_salloc := Semantics.with_salloc.

  Fixpoint app_traces (a1 a2 : abstract_trace) : abstract_trace :=
    match a1 with
    | empty => a2
    | cons_IO i a1' => cons_IO i (fun o => app_traces (a1' o) a2)
    | cons_branch b a1' => cons_branch b (app_traces a1' a2)
    | cons_read sz a a1' => cons_read sz a (app_traces a1' a2)
    | cons_write sz a a1' => cons_write sz a (app_traces a1' a2)
    | cons_salloc a1' => cons_salloc (fun w => app_traces (a1' w) a2)
    end.

  Definition oat a1 := option_map (app_traces a1).

  Lemma app_traces_assoc (a1 a2 a3 : abstract_trace) :
    app_traces (app_traces a1 a2) a3 = app_traces a1 (app_traces a2 a3).
  Proof. Abort.
  ...
  Fixpoint option_leak_set_reg_range_to_vars(fpval: word)(srcs: list Z) a : option abstract_trace :=
    match srcs with
    | nil => a
    | x :: xs => option_leak_set_reg_range_to_vars fpval xs (option_map (leak_set_reg_to_var fpval x) a)
    end.

  Lemma option_leak_set_reg_range_to_vars_correct fpval srcs a :
    option_leak_set_reg_range_to_vars fpval srcs a = option_map (leak_set_reg_range_to_vars fpval srcs) a.
  Proof.
    generalize dependent a.
    induction srcs; intros a'; destruct a'; simpl; try rewrite IHsrcs; reflexivity.
  Qed.
  ...
  Fixpoint option_leak_set_vars_to_reg_range(fpval: word) (dests: list Z) a : option abstract_trace :=
    match dests with
    | nil => a
    | x :: xs => option_map (leak_set_var_to_reg fpval x) (option_leak_set_vars_to_reg_range fpval xs a)
    end.

  Lemma option_leak_set_vars_to_reg_range_correct fpval srcs a :
    option_leak_set_vars_to_reg_range fpval srcs a = option_map (leak_set_vars_to_reg_range fpval srcs) a.
  Proof.
    generalize dependent a.
    induction srcs; intros a'; destruct a'; simpl; try rewrite IHsrcs; reflexivity.
  Qed.
  ...
  (*Fixpoint leak_set_vars_to_reg_range_tailrec(do_first: option abstract_trace)(fpval: word)(dests: list Z) : option abstract_trace :=
    match dests with
    | nil => do_first
    | x :: xs => leak_set_vars_to_reg_range_tailrec
                   (oat (do_first) (leak_set_var_to_reg fpval x)) fpval xs
    end.*)
                  ...
                  (cons_salloc
                     (fun fpval' =>
                        leak_set_vars_to_reg_range fpval' params
                          (transform_stmt_trace fpval' fbody t
                             (fun t' =>
            ...
            | cons_IO i g =>
                leak_set_reg_range_to_vars fpval argvars
                  (cons_IO i
                     (fun o =>
                        let t' := g o in
                        leak_set_vars_to_reg_range fpval resvars (f t')))
            | _ => empty
            end
        end
    end.
  ...
  Lemma load_iarg_reg_correct(i: Z): forall r e2 t1 t2 m1 m2 l1 l2 mc2 a2 fpval post frame maxvar v,
      ...
      exec e2 (load_iarg_reg i r) t2 m2 l2 mc2 (option_map (leak_load_iarg_reg fpval r) a2) post.
  Proof.
    intros.
  ...
  Lemma load_iarg_reg_correct'(i: Z): forall r e2 t1 t2 m1 m2 l1 l2 mc1 mc2 a1 a2 post frame maxvar v fpval,
      ...
      exec e2 (load_iarg_reg i r) t2 m2 l2 mc2 (option_map (leak_load_iarg_reg fpval r) a2)
  ...
  Lemma load_iarg_reg_correct''(i: Z): forall r e2 t1 t2 m1 m2 l1 l2 mc2 a2 frame maxvar v fpval,
      ...
      exec e2 (load_iarg_reg i r) t2 m2 l2 mc2 (option_map (leak_load_iarg_reg fpval r) a2)
        (fun t2' m2' l2' mc2' a2' =>
           t2' = t2 /\ m2' = m2 /\ l2' = map.put l2 (iarg_reg i r) v /\ a2' = a2 /\
             related maxvar frame fpval t1 m1 l1 t2' m2' l2').
  Proof.
    intros.
    unfold load_iarg_reg, leak_load_iarg_reg, stack_loc, iarg_reg, related in *. fwd.
  ...
  (* SOp does not create an up-to-date `related` before we invoke this one, because after SOp,
     ...
     yet in l1 and l2. *)
  Lemma save_ires_reg_correct: forall e t1 t2 m1 m2 l1 l2 mc1 mc2 a1 a2 x v maxvar frame post fpval,
      post t1 m1 (map.put l1 x v) mc1 a1 ->
      related maxvar frame fpval t1 m1 l1 t2 m2 l2 ->
  ...
  Proof.
    ...
    - simpl. unfold leak_set_var_to_reg, set_var_to_reg, stack_loc.
  ...
  Lemma set_reg_range_to_vars_correct:
    forall args argvs start e t1 t2 m1 m2 l1 l2 mc2 a2 maxvar frame post fpval,
      related maxvar frame fpval t1 m1 l1 t2 m2 l2 ->
      (List.length args <= 8)%nat ->
      a0 <= start ->
      start + Z.of_nat (List.length args) <= a7 + 1 ->
      ...
      (forall l2' mc2',
          ...
          map.getmany_of_list l2' (List.unfoldn (Z.add 1) (List.length args) start) = Some argvs ->
          post t2 m2 l2' mc2' a2) ->
      exec e (set_reg_range_to_vars start args) t2 m2 l2 mc2 (option_leak_set_reg_range_to_vars fpval args a2) post.
  Proof.
    induction args; intros.
    ...
    - simpl. unfold set_reg_to_var, leak_set_reg_to_var, stack_loc.
      ...
      + eapply exec.seq_cps.
        ...
        * rewrite option_map_id. eapply H5. 2: {
            cbn [List.unfoldn].
  ...
  Proof.
    ...
    - (* exec.store *)
      ...
      { unfold Memory.store, Memory.store_Z, Memory.store_bytes.
        ...
        erewrite map.getmany_of_tuple_in_disjoint_putmany; eauto. }
      do 5 eexists. split. 2: split; [reflexivity|eassumption].
      unfold related.
      repeat match goal with
             | |- exists _, _ => eexists
             | |- _ /\ _ => split
    ...
    - (* exec.stackalloc *)
      ...
      intros.
      eapply exec.seq_cps.
      edestruct grow_related_mem as (mCombined1 & ? & ?).
      1,2: eassumption.
      ...
      exists (S F').
      destruct H9p2 as [[t1'' [IHp1 IHp2]] [a1' [t1''0 [t2'' [IHp3 [IHp4 [IHp5 IHp6]]]]]]].
    ...
    Search a1. assumption.
      ...
      Search (Semantics.abs_tr_eq (Semantics.app_traces _ _) _).
      eapply Semantics.abs_tr_eq_trans.
      { apply Semantics.abs_tr_eq_symm. apply app_traces_assoc. }
      eapply Semantics.abs_tr_eq_app.
      { apply Semantics.abs_tr_eq_self. }
      { cbn [Semantics.generator Semantics.app_traces]. constructor.
      eapply Semantics.abs_tr_eq_correct2 in IHp3.
      2: { eapply Semantics.abs_tr_eq_symm. eapply app_traces_assoc. }
      ...
      exists mSmall'. exists mStack'. Search Memory.anybytes. split; [exact IHp2p2|]. Search m1'. Search m'. unfold related in IHp1. split; [assumption|].
        ...
        { intros.
          ...
          intros. simpl in H6. destruct H6 as [H6p1 [mSmall' [mStack' [H6p2 [H6p3 [H6p4 H6p5]]]]]].
          eassert (H6: exists _ _, _).
            { exists mSmall'. exists mStack'. instantiate (1 := _ /\ _ /\ _ /\ _ /\ _).
              split; [exact H6p1|].
              split; [exact H6p2|].
              split; [exact H6p3|].
              split; [exact H6p4|].
              instantiate (1 := match a1 with
                                | Some a1 => exists f, a1 = Semantics.cons_salloc f
                                | None => True end).
              destruct a1; [|reflexivity].
              destruct H6p5 as [t1'' [a1' [CTp1 CTp2]]].
              destruct H6p1 as [t''0 Ht''0]. subst.
              rewrite app_one_cons in CTp2. rewrite app_assoc in CTp2. apply app_inv_tail in CTp2. subst.
              rewrite rev_app_distr in CTp1. simpl in CTp1. inversion CTp1. subst.
              eexists. reflexivity.
            }
          split; [exact H6|].
          
          instantiate (1 := match a1 with
                               | Some (Semantics.cons_salloc f) => Some (f a)
                               | _ => None
                               end).
          destruct a1; [|reflexivity].
          destruct H6p5 as [t1'' [a1' [CTp1 CTp2]]].
      ...
      1,4,3,2: eassumption.
      destruct a1; [|reflexivity].
      destruct H6p1p4 as [f H6p1p4]. subst.
      destruct H6p2 as [F' [a1' [t1'' [t2'' [CTp1 [CTp2 [CTp3 CTp4]]]]]]].
      apply app_inv_tail in CTp2. subst.
      exists (S F'). eexists. exists (t1'' ++ [Semantics.salloc a]). eexists.
      split.
      { rewrite rev_app_distr. cbn [rev List.app]. econstructor. eassumption. }
    ...
    - (* SOp *)
      ...
      unfold exec.lookup_op_locals in *.
      eapply exec.seq_cps. eapply load_iarg_reg_correct; (blia || eassumption || idtac).
      clear mc2 H1. intros. destruct_one_match; fwd.
      {
        eapply exec.seq_cps. eapply exec.seq_cps.  eapply load_iarg_reg_correct; (blia || eassumption || idtac).
        clear mc2 H0. intros.
        eapply exec.op.
        ...
        { eapply save_ires_reg_correct''; try (eassumption || blia).
          ...
          split; [reflexivity|].
          split.
          2: { subst t2'1. subst t2'0. subst t2'. repeat rewrite app_assoc. reflexivity. }
          intros. destruct fuel as [|fuel']; [blia|].
          cbn [transform_stmt_trace].
          repeat rewrite rev_app_distr. repeat rewrite rev_involutive.
          eapply Semantics.abs_tr_eq_correct1.
    ...
    - inversion H; subst.
      + (*if true*) clear IHs1_2.
           ...
           { eapply IHs1_1.
             { eapply exec.weaken.
               { eapply exec.intersect.
                 { eapply trace_prefix. eapply H10; eassumption. }
                 { eapply H10; eassumption. } }
               simpl. intros. destruct H3 as [Hprefix [Hpost CT]].
               split.
               { eassert (thisPost: _ /\ _ /\ _).
                 { split; [eapply Hprefix|].
                   split; [eapply Hpost|].
                   instantiate (1 := match a1 with
                                     | Some a1 => exists a1', a1 = Semantics.cons_branch true a1'
                                     | None => True end).
                   destruct a1 as [a1|]; [|reflexivity].
                   destruct CT as [t1'' [a1' [CTp1 CTp2]]]. destruct Hprefix as [t'' Hprefix]. subst.
                   rewrite app_one_cons in CTp2. rewrite app_assoc in CTp2. apply app_inv_tail in CTp2. subst.
                   rewrite rev_app_distr in CTp1. cbn [rev List.app] in CTp1. inversion CTp1. subst.
                   eexists. reflexivity. }
                 exact thisPost. }
               instantiate (1 := match a1 with
                                 | Some (Semantics.cons_branch true a1') => Some a1'
                                 | _ => None
                                 end).
             destruct a1 as [a1|]; [|reflexivity].
             destruct CT as [t1'' [a1' [CTp1 CTp2]]]. destruct Hprefix as [t'' Hprefix]. subst.
             ...
             { eassumption. } }
           simpl. intros.
           destruct H3 as [t1' [m1' [l1' [mc1' [H3p1 H3p2]]]]].
           exists t1'. exists m1'. exists l1'. exists mc1'.
           split; [apply H3p1|].
           clear H3p1. destruct H3p2 as [[H3p1 [H3p2 H3p3]] H3p4].
           split; [apply H3p2|]. clear H3p2.
           destruct a1 as [a1|]; [|reflexivity].
           destruct H3p1 as [t'' H3p1]. destruct H3p3 as [a11 H3p3]. subst.
           destruct H3p4 as [F [a1' [t1'' [t2'' [H3p1 [H3p2 [H3p3 H3p4]]]]]]].
           apply app_inv_tail in H3p2. subst.
           exists (S F). exists a1'. eexists. eexists.
           split; [|split].
           2: { rewrite app_one_cons. rewrite app_assoc. reflexivity. }
           { rewrite rev_app_distr. cbn [rev List.app]. constructor. assumption. }
           split.
           2: { subst t2'0. subst t2'. rewrite app_one_cons. repeat rewrite app_assoc. reflexivity. }
           intros. destruct fuel as [|fuel']; [blia|].
           cbn [transform_stmt_trace].
           repeat rewrite rev_app_distr. repeat rewrite rev_involutive. cbn [rev List.app].
           cbv [leak_prepare_bcond leak_spill_bcond]. repeat rewrite app_nil_r.
           cbn [List.app]. rewrite (app_one_cons _ (rev t2'')). repeat rewrite app_assoc.
           eapply Semantics.generates_with_rem_trans.
           { eapply Semantics.generates_with_empty_rem_app.
             apply Semantics.generates_generates_with_empty_rem.
             apply Semantics.generator_generates. }
        ...
        (* begin thing copy-pasted from above *)
        eapply exec.weaken.
           { eapply IHs1_1.
             { eapply exec.weaken.
               { eapply exec.intersect.
                 { eapply trace_prefix. eapply H10; eassumption. }
                 { eapply H10; eassumption. } }
               simpl. intros. destruct H2 as [Hprefix [Hpost CT]].
               split.
               { eassert (thisPost: _ /\ _ /\ _).
                 { split; [eapply Hprefix|].
                   split; [eapply Hpost|].
             ...
             { eassumption. } }
           simpl. intros.
           destruct H2 as [t1' [m1' [l1' [mc1' [H3p1 H3p2]]]]].
           exists t1'. exists m1'. exists l1'. exists mc1'.
           split; [apply H3p1|].
           clear H3p1. destruct H3p2 as [[H3p1 [H3p2 H3p3]] H3p4].
           split; [apply H3p2|]. clear H3p2.
             ...
             { eapply exec.weaken.
               ...
               { eassert (thisPost: _ /\ _ /\ _).
                 { split; [eapply Hprefix|].
                   split; [eapply Hpost|].
                   instantiate (1 := match a1 with
                                     | Some a1 => exists a1', a1 = Semantics.cons_branch false a1'
                                     | None => True end).
                   destruct a1 as [a1|]; [|reflexivity].
                   destruct CT as [t1'' [a1' [CTp1 CTp2]]]. destruct Hprefix as [t'' Hprefix]. subst.
                   rewrite app_one_cons in CTp2. rewrite app_assoc in CTp2. apply app_inv_tail in CTp2. subst.
                 ...
                 exact thisPost. }
               instantiate (1 := match a1 with
                                 | Some (Semantics.cons_branch false a1') => Some a1'
                                 | _ => None
                                 end).
             destruct a1 as [a1|]; [|reflexivity].
             destruct CT as [t1'' [a1' [CTp1 CTp2]]]. destruct Hprefix as [t'' Hprefix]. subst.
           ...
           { eapply IHs1_2.
             { eapply exec.weaken.
               { eapply exec.intersect.
                 { eapply trace_prefix. eapply H10; eassumption. }
                 { eapply H10; eassumption. } }
               simpl. intros. destruct H1 as [Hprefix [Hpost CT]].
               split.
               { eassert (thisPost: _ /\ _ /\ _).
                 { split; [eapply Hprefix|].
                   split; [eapply Hpost|].
             ...
             { eassumption. } }
           simpl. intros.
           destruct H1 as [t1' [m1' [l1' [mc1' [H3p1 H3p2]]]]].
           exists t1'. exists m1'. exists l1'. exists mc1'.
           split; [apply H3p1|].
           clear H3p1. destruct H3p2 as [[H3p1 [H3p2 H3p3]] H3p4].
           split; [apply H3p2|]. clear H3p2.
    ...
    - Search outcome. (* exec.call *)
      ...
      *)
      ...
      eapply set_vars_to_reg_range_correct.
      { unfold related. eexists lStack1, lRegs1, _. ssplit.
        { rewrite Rp0. (*maybe there's a way to do this with less looking-ahead?*)reflexivity. }
        { eassumption. }
        { eassumption. }
        { eassumption. }
        { eassumption. }
      ...
      { intros m22 l22 mc22 t22 R22. do 4 eexists. split. 1: eassumption. split. 1: eassumption.
        ...
        { eapply Semantics.generates_with_rem_app. Search Semantics.generates_with_rem.
          apply Semantics.generates_generates_with_empty_rem.
          apply Semantics.generator_generates. }
        cbn [Semantics.app_traces]. cbn [List.app].
        constructor.
        eapply Semantics.generates_with_rem_trans.
    ...
    - (* exec.store *)
      ...
      destruct a1 as [a1|]; try reflexivity.
      Search post.
      apply H3 in H2. Check H2. destruct H2 as [t1'' [a1' [CTp1 CTp2]]].
      exists 1%nat. do 3 eexists.
      split; [eassumption|].
      split; [eassumption|].
      split.
    ...
    - (* exec.inlinetable *)
      ...
      + intros. do 4 eexists.
        ...
        { eassumption. }
        destruct a1 as [a1|]; [|reflexivity].
        apply H3 in H2. clear H3. Check H2.
        destruct H2 as [t1'' [a1' [CTp1 CTp2]]].
        exists 1%nat. do 3 eexists.
        split; [eassumption|].
        split; [assumption|].
        split.
