...
Section Spilling.
  ...
  Proof.
    ...
    eapply exec.weaken. {
      ...
      intros. simpl. subst a kL4.
      repeat (rewrite rev_app_distr in * || rewrite rev_involutive in * || cbn [rev List.app] in * ).
      repeat rewrite <- app_assoc.
      rewrite List.skipn_app_r by (symmetry; apply rev_length).
      reflexivity.
    }
    cbv beta. intros kL5 tL5 mL5 lFL5 mcL5 (kH5 & tH5 & mH5 & lFH5 & mcH5 & k1'' & R5 & OC & Ek1'' & CT).
    fwd.
  ...
  Proof. Search stransform_stmt_trace.
    ...
    - (* exec.loop *)
      ...
      + specialize H0 with (1 := H3p1). cbn in H0. fwd.
        ...
        * eapply exec.weaken. 1: eapply IH2.
          ...
          -- cbv beta. intros. fwd. eapply exec.weaken.
             ...
             ++ cbv beta. intros.
                destruct H5 as [k1' [t1'1 [m1'1 [l1'1 [mc1'1 [k1''1 [R [Hpost [E1 CT]]]]]]]]].
                subst. do 6 eexists.
                split; [eassumption|]. split; [eassumption|]. split.
                ...
                rewrite stransform_stmt_trace_step. simpl. rewrite H3p3.
                rewrite List.skipn_app_r by reflexivity. cbn [Let_In_pf_nd]. simpl in *.
                repeat (rewrite rev_app_distr in * || rewrite rev_involutive in * || cbn [rev List.app] in * ).
                repeat rewrite <- app_assoc in *. rewrite H5p3.
      ...
      + specialize H0 with (1 := H3p1). cbn in H0. fwd.
        ...
        split; intros.
        * do 6 eexists. split; [|split].
          2: { Search post. eapply H1. 1: eassumption. cbn. rewrite E. congruence. }
          { eassumption. } split.
          { rewrite app_one_cons. rewrite app_assoc. reflexivity. }
          intros.
          repeat (rewrite rev_app_distr in * || rewrite rev_involutive in * || cbn [rev List.app] in * ).
    ...
    - (* exec.seq *)
      ...
      + cbn. intros. fwd. eapply exec.weaken.
        -- eapply IH2; eauto. intros.
           replace (rev k1''0 ++ k1'' ++ k) with (rev (rev (rev k1''0 ++ k1'')) ++ k).
           2: { rewrite rev_involutive. rewrite <- app_assoc. reflexivity. }
           rewrite H4. rewrite stransform_stmt_trace_step. simpl.
           repeat (rewrite rev_involutive in * || rewrite rev_app_distr in * || rewrite <- app_assoc in * ).
           rewrite H1p3. rewrite List.skipn_app_r by reflexivity. reflexivity.
        -- cbv beta. intros. destruct H1 as [k1' [t1'1 [m1'1 [l1'1 [mc1'1 [t1''1 [R [Hpost [E1 CT]]]]]]]]]. subst.
           do 6 eexists. split; [eassumption|]. split; [eassumption|]. split.
           { rewrite app_assoc. reflexivity. } intros.
           repeat (rewrite rev_involutive in * || rewrite rev_app_distr in * || rewrite <- app_assoc in * ).
           rewrite stransform_stmt_trace_step. simpl. rewrite H1p3.
           rewrite List.skipn_app_r by reflexivity. rewrite CT. reflexivity.
           
    - (* exec.skip *)
      eapply exec.skip. do 6 eexists. repeat (split; [eauto|]).
      { instantiate (1 := nil). reflexivity. }
