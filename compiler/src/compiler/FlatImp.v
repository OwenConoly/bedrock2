...
Require Import bedrock2.Semantics. (*because it contains trace_alignment. better place to put trace_alignment? *)
  ...
  Section FlatImpExec.
    ...
    | stackalloc: forall k t mSmall l mc x n body post,
        ...
        (forall mStack mCombined,
            ...
            exec body (leak_unit :: k) t mCombined (map.put l x a) (addMetricLoads 1 (addMetricInstructions 1 mc))
    ...
    Proof.
      intros H. induction H; try (econstructor; intuition eauto; eexists; trace_alignment; fail).
      - econstructor; intuition eauto. specialize H2 with (1 := H3). fwd.
        eexists. intuition eauto. eexists. trace_alignment.
      ...
      - clear H2 H4. econstructor; intuition eauto; fwd; eauto.
        ...
        { eapply weaken. 1: eapply H3; eauto. simpl. intros. fwd.
          ...
          simpl. intuition. eexists. trace_alignment. }
        simpl in *. fwd. eapply weaken. 1: eapply H5; eauto.
        simpl. intros. fwd. intuition. eexists. trace_alignment.
    ...
    Proof.
      ...
      - econstructor. 4: eapply exec_extends_trace. all: intuition eauto.
        { eapply IHexec. intros.
          repeat (rewrite app_assoc || rewrite (app_one_l _ k)). auto. }
      ...
      - eapply if_true; eauto. eapply IHexec.
        intros. repeat (rewrite app_assoc || rewrite (app_one_l _ k)). auto.
      ...
      - clear H2 H4. eapply loop. 1: eapply exec_extends_trace. all: intuition eauto; fwd; eauto.
        { eapply weaken. 1: eapply exec_extends_trace.
          ...
          instantiate (1 := fun k'0 t'0 m'0 l'0 mc'0 =>
                              mid2 k'0 t'0 m'0 l'0 mc'0 /\ exists k'', k'0 = k'' ++ k).
          simpl. intuition eauto. eexists. trace_alignment. }
    ...
    Proof.
      ...
      - econstructor; intuition eauto. apply H2 in H3. fwd.
        eexists. intuition eauto. eexists. intuition eauto. 1: trace_alignment.
        auto.
      - econstructor; intuition eauto.
        ...
        split; [trace_alignment|]. repeat rewrite <- app_assoc. auto.
      - econstructor; intuition eauto. eexists. split; [trace_alignment|]. auto.
      - econstructor; intuition eauto. eexists. split; [trace_alignment|]. auto.
      - econstructor; intuition eauto. eexists. split; [trace_alignment|]. auto.
      ...
      - eapply loop. 1: eapply IHexec. all: intuition eauto; fwd; eauto.
        { eexists. split; [trace_alignment|]. rewrite app_nil_r. simpl. auto. }
        { eapply exec_ext with (pick_sp1 := _).
          { eapply weaken. 1: eapply H3; eauto. simpl. intros.
            ...
            fwd. eexists. split; [trace_alignment|]. rewrite app_nil_r.
        ...
        { eapply weaken. 1: eapply H5; eauto. simpl. intros. fwd.
          eexists. split; [trace_alignment|]. rewrite app_nil_r.
          repeat rewrite <- app_assoc. auto. }
        solve_picksps_equal.
